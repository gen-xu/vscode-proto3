...
export class Proto3Compiler {
  ...
  public compileAllProtos() {
    ...
    this._config.getAllProtoPaths().forEach((proto) => {
      this.runProtoc(args.concat(proto), { cwd: this._config.workspaceFolder.uri.fsPath }, (stdout, stderr) => {
  ...
  public compileProtoToTmp(document: vscode.TextDocument, callback?: (stderr: string) => void) {
    let proto = path.relative(vscode.workspace.getWorkspaceFolder(document.uri).uri.fsPath, document.fileName);

    let args = this._config.getProtoPathOptions().concat(this._config.getTmpJavaOutOption(), proto);

    this.runProtoc(args, { cwd: vscode.workspace.getWorkspaceFolder(document.uri).uri.fsPath }, (stdout, stderr) => {
      if (callback) {
        callback(stderr);
      }
    });
