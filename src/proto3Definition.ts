...
export class Proto3DefinitionProvider implements vscode.DefinitionProvider {
  ...
  public async provideDefinition(
    ...
    if (matchedGroups && matchedGroups.length == 5) {
      ...
      const location = this.findImportDefinition(importFilePath, document);
      if (location) {
        return location;
      }
  ...
  private async findEnumOrMessageDefinition(document: vscode.TextDocument, target: string): Promise<vscode.Location> {
    const searchPaths = await Proto3Import.getImportedFilePathsOnDocument(document);
    const files = [document.uri.fsPath, ...searchPaths];
  ...
  private async findImportDefinition(importFileName: string, importBy: vscode.TextDocument): Promise<vscode.Location> {
    let importPath = "";
    const filePath = path.join(vscode.workspace.getWorkspaceFolder(importBy.uri).uri.fsPath, importFileName);
    if (fs.existsSync(filePath)) {
      importPath = filePath;
    } else {
      const files = await fg(
        path.join(vscode.workspace.getWorkspaceFolder(importBy.uri).uri.fsPath, "**", importFileName)
      );
      importPath = files[0];
    }
