...
export function activate(ctx: vscode.ExtensionContext): void {
  let channel = vscode.window.createOutputChannel("vscode-proto3");
  ctx.subscriptions.push(
    vscode.languages.registerCompletionItemProvider(PROTO3_MODE, new Proto3CompletionItemProvider(), ".", '"')
  );
  ctx.subscriptions.push(
    vscode.languages.registerDefinitionProvider(PROTO3_MODE, new Proto3DefinitionProvider(channel))
  );
    ...
    provideDocumentFormattingEdits(document: vscode.TextDocument): vscode.TextEdit[] {
      ...
      switch (document.uri.scheme) {
        case "untitled": // File has not yet been saved to disk use workspace path
          opts["cwd"] = vscode.workspace.getWorkspaceFolder(document.uri).uri.fsPath;
